/* FAST and Optical Flow for Gumtix
 * Gum_FAST_OF.cpp
 *
...
//g++ `pkg-config --cflags opencv` -o test.bin Gum_FAST_OF.cpp `pkg-config --libs opencv` overo_stm_spi_simplex.c
...
// 		start_timer();
		...
		uint8_t numCorners = corners.size();
		if (numCorners == 0){
			xmedian = 0;
			ymedian = 0;
		}
			...
			int32_t dx[numCorners];
			int32_t dy[numCorners];
			for( int8_t i = 0; i < numCorners; ++i) {
				dx[i] = (int32_t)((cornersA[i].x - cornersB[i].x)*(1<<6));
				dy[i] = (int32_t)((cornersA[i].y - cornersB[i].y)*(1<<6));
//				xT = xT + dx[i];
//				yT = yT + dy[i];
			}
	...
	//		y_ave = yT/numCorners;
			...
			for(uint8_t n=0;n<(numCorners-1);n++){
				for(uint8_t m=n+1;m<numCorners;m++){
					if(dx[n]>dx[m]){
						temp=dx[n];
						dx[n]=dx[m];
						dx[m]=temp;
